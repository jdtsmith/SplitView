...
Important points:
* `SplitView` relies on the undocumented `spaces` API, which _must_ both be installed separately for it to work; see https://github.com/asmagill/hs._asm.undocumented.spaces.
* `removeDesktop` functionality requires the separate accessibility ui controls to be installed; see https://github.com/asmagill/hs._asm.axuielement/.  `SplitView` will still function without it, but you won't be able to remove fullscreen spaces.
